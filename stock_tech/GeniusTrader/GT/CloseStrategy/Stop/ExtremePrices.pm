...
use GT::Prices;
use GT::Indicators::Generic::Min;
use GT::Indicators::Generic::Max;
...
sub initialize {
    ...
    $self->{'min'} = GT::Indicators::Generic::Min->new([ $self->{'args'}->get_arg_names(1) ]);
    $self->{'max'} = GT::Indicators::Generic::Max->new([ $self->{'args'}->get_arg_names(1) ]);
